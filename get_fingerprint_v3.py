...
def get_fingerprint(model, model_name, detail, result_dir):
    ...
    # if model_name == 'SRDD':
    #     input_x_hr = input_x_hr*255.0.clamp(0, 255).round()
    ...
    for epoch in range(iters_all):
        ...
        if model_name == 'DCLS':
            ...
            # model.feed_data(input_x_lr.to(device),input_x_lr.to(device))
            # model.test_with_grad()
            # visuals = model.get_current_visuals_with_grad()
            # output_x = visuals["Fingerprint_SR"].to(device)
        ...
        # loss = mse_loss(output_x, input_x_hr) - 0.01*tv_loss(input_x_hr)
        loss = mse_loss(output_x, input_x_hr) - 1e-3*tv_loss(input_x_hr)
    ...
    x_npy = input_x_hr.detach().cpu().numpy()
    np.save(result_dir +'/1_total/1_' + model_name + '_' + detail + '.npy', x_npy)
